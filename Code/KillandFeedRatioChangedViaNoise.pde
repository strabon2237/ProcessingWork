...
float d1 = 1;
float d2 = 0.5;
float dt = 1; // smaller for stability
int scale = 2;
int rows, columns;

float noiseScaleF = 0.05; // scale for feed noise
float noiseScaleK = 0.05; // scale for kill noise
...
void update(){
  for(int r = 0; r < rows; r++){
    for(int c = 0; c < columns; c++){  
      //Noise used to change the feed and kill rate throughout the rows/columns
      float localF = map(noise(c * noiseScaleF, r * noiseScaleF), 0, 1, 0.01, 0.1);
      float localK = map(noise(c * noiseScaleK, r * noiseScaleK + 1000), 0, 1, 0.045, 0.07);
      ...
      // Clamp values to [0,1] to prevent black screen
